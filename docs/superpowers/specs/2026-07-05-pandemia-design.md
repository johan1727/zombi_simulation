...
| **Media — ciclo de poses** (pendiente, Plan 7) | Igual que arriba + caminata/pánico/cojera REAL | **Ciclo de poses instanciado**: hornear N poses adicionales (caminar, correr, cojear) desde los clips ya convertidos (`survivor-anim-{idle,run,jump}.glb`); cada ciudadano elige su pose por frame según su fase de movimiento (determinista, calculado en el render — CERO cambio a `src/sim/`). El pipeline de horneado (`poseBake.ts`) ya está listo — esta task futura solo necesita hornear más frames y ciclar entre pools, no rehacer el resto. | Plan 7 |
