...
- [ ] **Step 1: Test que falla — `tests/buildingModels.test.ts`** (solo la función pura, sin cargar GLB real — vitest no tiene DOM/WebGL):
...
- [ ] **Step 2-4:** TDD para la función pura; `cityView.ts` se verifica en
...
- [ ] **Step 5: Commit** — `feat: edificios de fondo con modelos reales de Kenney (Plan 6)`
...
- [ ] **Step 1: Implementar** (sin test unitario obligatorio — puramente
...
- [ ] **Step 3: Verificación en navegador** — autos visibles en las calles,
...
- [ ] **Step 4: Commit** — `feat: autos decorativos en las calles (Plan 6)`
...
- [ ] **Step 1: Test que falla — `tests/poseBake.test.ts`** (sin WebGL real; usa un `SkinnedMesh` sintético mínimo — 2 vértices, 2 huesos, para verificar la matemática sin depender de cargar un GLB):
...
- [ ] **Step 2-4:** TDD para `hornearPose`; el resto (`personajesView.ts`,
...
- [ ] **Step 5: Commit** — `feat: ciudadanos y zombis con silueta real horneada de un solo frame (Plan 6)`
...
- [ ] **Step 1:** `npm test` completo en verde, `npx tsc --noEmit` limpio.
- [ ] **Step 2: Verificación de rendimiento en navegador** — partida completa
...
- [ ] **Step 3: Cierre** — lecciones condensadas en CLAUDE.md (mantener el
